...
@section('title', app_name() . ' | ' . __('labels.backend.access.Home.servdep3'))
        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.backend.access.Home.servdep3')}} <small class="text-muted">{{ __('labels.backend.access.Home.Mainsectionf.active') }}</small>
                </h4>
            </div>
            <!--col-->
        </div>
...
<div class="form-group">
                    ...
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="paragraph" name="paragraph" placeholder="{{trans('forms.Main.Placeholder.feild')}}"  required>
                    </div>
                </div>
...
<script>
  ...
  $('#Main_section').DataTable({
        processing: false,
        serverSide: true,
        ajax: "{{ url('admin/ShowServiceeThree') }}",
        columns: [
...
// Edit-function
    function editFunc(id){
        ...
        $.ajax({
            type:"POST",
            url: "{{url('admin/Edit-ServiceeThree')}}",
            data:{id:id},
            dataType:'json',
            success:function(res)
            {
                ...
                $('#paragraph').val(res.Paragraph1.lang);
            }
});
} 
...
// submit Button Function
    $('#CompanyForm').submit(function(e) {
            ...
            $.ajax({
            type:'POST',
            url: "{{ url('admin/Store-ServiceeThree')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
