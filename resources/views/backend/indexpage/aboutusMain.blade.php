...
@section('title', app_name() . ' | ' . __('labels.backend.access.Home.aboutusM'))
        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.backend.access.Home.aboutusM')}} <small class="text-muted">{{ __('labels.backend.access.Home.Mainsectionf.active') }}</small>
                </h4>
            </div>
            <!--col-->
        </div>
...
<script>
  ...
  $('#Main_section').DataTable({
        processing: false,
        serverSide: true,
        ajax: "{{ url('admin/ShowAboutUsMain') }}",
        columns: [
            { data: 'id', name: 'id' },
...
// Edit-function
    function editFunc(id){
        ...
        $.ajax({
            type:"POST",
            url: "{{url('admin/Edit-AboutUsMain')}}",
            data:{id:id},
            dataType:'json',
...
// submit Button Function
    $('#CompanyForm').submit(function(e) {
            ...
            $.ajax({
            type:'POST',
            url: "{{ url('admin/Store-AboutUsMain')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
