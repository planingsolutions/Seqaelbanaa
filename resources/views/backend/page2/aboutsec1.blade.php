        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.backend.access.Home.Section1') }} <small class="text-muted">{{ __('labels.backend.access.Home.Mainsectionf.active') }}</small>
                </h4>
            </div>
            <!--col-->
        </div>
                    ...
                    <table id="Main_section" class="table">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th>{{ trans('labels.backend.access.Home.aboutsec1.Paragraph1') }}</th>
                                <th>{{ trans('labels.backend.access.Home.aboutsec1.Paragraph2') }}</th>
...
<!-- Tittle -->
                <div class="form-group">
                    <label for="Tittle" class="col-sm-12 control-label">{{trans('forms.Main.Label.Aboutus Paragrpah1')}}</label>
                    <div class="col-sm-12">
...
<!-- Description -->
                <div class="form-group">
                    <label for="Description" class="col-sm-12 control-label">{{trans('forms.Main.Label.Aboutus Paragrpah2')}}</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="component" name="Paragraph2" placeholder="{{trans('forms.Main.Placeholder.Description')}}" maxlength="255" required>
                    </div>
                </div>
...
// Edit-function
    function editFunc(id){
        ...
        $.ajax({
            type:"POST",
            url: "{{url('admin/Edit1-AboutussectionTable')}}",
            data:{id:id},
            dataType:'json',
...
function deleteFunc(id){
    ...
    $.ajax({
    type:"POST",
    url: "{{ url('admin/Delete1-AboutussectionTable') }}",
    data: { id: id },
    dataType: 'json',
    success: function(res){
...
// submit Button Function
    $('#CompanyForm').submit(function(e) {
            ...
            $.ajax({
            type:'POST',
            url: "{{ url('admin/Store1-AboutussectionTable')}}",
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
